...
class SearchMasterServer(port: Int, id: Int) extends AbstractSearchServer(port, id) {
  ...
  override def query(q: String) = {
    ...
    responses.map {responses => queryResponse(responses
      .filter(response => (response.getStatus() == HttpResponseStatus.OK) && !response.getContent.toString(UTF_8).isEmpty())
      .map(response => response.getContent().toString(UTF_8)))
    }
  }
}
